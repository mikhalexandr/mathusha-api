...
def admin_required(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        headers = {
            "Authorization": f"client_id={os.getenv('KEYCLOAK_CLIENT_ID')}, client_secret={os.getenv('CLIENT_SECRET_KEY')}",
            "Content-Type": "application/x-www-form-urlencoded"
        }
        data = {"token": token}
        response = requests.post(f"{os.getenv('KEYCLOAK_SERVER_URL')}/realms/{os.getenv('USER_REALM_NAME')}"
